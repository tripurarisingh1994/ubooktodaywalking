...
export class HomePage {
  ...
  private url = "https://ubooktoday.com/android/walkin?userid=";
  // private url = "http://staging.ubooktoday.com/android/walkin?userid=";
  ...
  constructor(
    ...
    // this.countryCode = this.todo1.controls['countryCode'];
  ...
  logForm1() {
    let unmaskedData = {
      mob: this.mob.replace(/\D+/g, ''),
     
  };
  console.log(unmaskedData);

    this.newCust.newCustReg(this.todo1.value.firstname, this.todo1.value.lastname, this.todo1.value.email, this.todo1.value.mobile, this.spaid_from_spalogin, this.country_Code).subscribe(data => {
      console.log(data);
      if (data['message'] == 'Successful') {

        this.existCust.http.get(this.url + unmaskedData.mob).subscribe(data => {
          console.log(data)
          if (data['message'] == 'Please Enter Valid Details') {
            this.presentAlert("ubooktodaywalking", data['message']);
          }
            ...
            this.em = '';
            this.mob = '';
            this.country_Code = this.countryC[188].code;
  ...
  trimLastCharacter(value) {
    ...
    if (value.length > 14) {
      this.todo1.patchValue({mobile: value.slice(0, -1)})
    }
  }

  normalMobileTrim(value) {
    if(value.length > 12) {
      this.todo1.patchValue({mobile: value.slice(0, -1)})
    } 
  }

  exit_MobileTrim(value) {
    if(value.length > 12) {
      this.todo.patchValue({email: value.slice(0, -1)})
    } 
  }
